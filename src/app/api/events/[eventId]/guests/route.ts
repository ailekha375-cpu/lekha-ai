...
function getGuestsUrl(eventId: string) {
  ...
  return `${origin}/api/events/${eventId}/guests${search}`;
}

async function proxyGuests(
  request: NextRequest,
  eventId: string,
  init: RequestInit
) {
  ...
  const res = await fetch(getGuestsUrl(eventId), { ...init, headers });
...
export async function GET(
  ...
  try {
    ...
    return await proxyGuests(request, eventId, { method: 'GET' });
  } catch (err) {
...
export async function POST(
  ...
  try {
    ...
    return await proxyGuests(request, eventId, {
      method: 'POST',
